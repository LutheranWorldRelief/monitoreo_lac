...
//                else
    ...
    public function afterValidate() {
        ...
        foreach ($this->_process_relation as $name => $process) {
            ...
            if ($relation->multiple) {
                ...
                foreach ($children as $index => $child) {
                    ...
                    if (!$child->validate()) {
                        ...
                        $errors = $child->getFirstErrors();
                        $llaves = $relation->link;
                        foreach ($llaves as $from => $to) {
                            if (isset($errors[$from]))
                                unset($errors[$from]);
                        }
                        if (count($errors) > 0) {
                            $this->_relatedErrors[$name][$index] = $errors;
            ...
            if ($error)
                $this->owner->addError($name, 'Error en las relaciones');
        }
    }
