...
//            ->andFilterWhere(['event.organization_id' => $organizationId]);
    ...
    public function actionApiFusion()
    {
        ...
        if (count($models) < 2)
            throw new HttpException(500, Yii::t('app', 'No se cargaron los modelos necesarios para la fusión'));
        ...
        if (!$model)
            throw new HttpException(500, Yii::t('app', 'No se logró identificar al modelo principal de la fusión'));
        ...
        try {
            ...
            if (!$saved)
                throw new HttpException(500, Yii::t('app', 'No se logró guardar el modelo principal con los cambios'));
            else {
                foreach ($models as $key => $m) {
                    $mid = $m->id;
